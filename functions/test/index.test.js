...
describe('Github api HTTP operations test using Mock server (Nock)', function(done) {
  ...
  it('should add all labels from a array', function (done) {
    ...
    .reply(function(uri, requestBody) { // The reply function
      //console.log('path:', this.req)
      //console.log('headers:', requestBody)
      expect(JSON.parse(requestBody).name).to.equal(labelArray[0].name) //Expected input - we check on name instead of the whole object
    }) 
    //.log(console.log)
  ...
  it('should execute the main method and perform addition and deletion using hardcoded labels', function (done) {
      ...
      .reply(function(uri, requestBody) { // The reply function
        ...
        // console.log('headers:', this.req.headers)
       // console.log('headers:', requestBody)
       expect(arrLabelsAddParsed).to.deep.include(JSON.parse(requestBody)) //Expected input - we check on name instead of the whole object
      })}
      //.log(console.log)
